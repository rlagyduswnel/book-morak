...
export default function EditPostPage() {
  ...
  const isDirty = rating !== initialRating || content !== initialContent;
  const canSubmit =
    rating >= 1 && content.trim().length >= 30 && !isSubmitting;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [post, csv] = await Promise.all([
      ...
      const foundBook = books.find((item) => item.isbn13 === post.bookId);

      setBook(foundBook ?? null);
      setRating(post.rating);
      setContent(post.content);
      setInitialRating(post.rating);
